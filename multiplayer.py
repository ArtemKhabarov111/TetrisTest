...
# Import existing components from the singleplayer code
...
class MultiplayerGame(Game):
    def __init__(self, get_next_shape, update_score, area_rect, controls):
        ...
        # Save the designated drawing area and control mapping.
        self.area_rect = area_rect  # pygame.Rect for the player's area
        self.controls = controls    # Dict with keys: 'left', 'right', 'rotate', 'down'
        # Define the playfield drawing offset inside the area with the same padding as singleplayer.
        self.field_offset = (self.area_rect.x + PADDING, self.area_rect.y + PADDING)
        # Reset self.rect so that the game field surface is drawn at (0,0) of its own surface.
        self.rect.topleft = (0, 0)
        # A flag to mark if this player has lost.
    ...
    def input(self):
        ...
        # Horizontal movement (if timer allows)
    ...
    def run(self):
        ...
        self.draw_grid()
        # Blit the playfield into the player's area using the field_offset.
    ...
    def draw_game_over_overlay(self, win_text=None):
        ...
        # Create a black background surface with 150 alpha for the player's area.
        black_area = pygame.Surface((self.area_rect.width, self.area_rect.height))
        black_area.set_alpha(150)  # Set black area's transparency to 150.
        black_area.fill("black")

        # Blit the black background first.
        self.display_surface.blit(black_area, (self.area_rect.x, self.area_rect.y))

        # Calculate center positions for text within the player's area.
...
class Multiplayer:
    def __init__(self):
        ...
        # Create next-shape lists for each player (always kept at length 3).
        ...
        # Create side panels for each player.
        self.score1 = Score()
        ...
        # For score, mimic the singleplayer positioning: below preview with a little extra padding.
        ...
        # Flag indicating that both players have lost.
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    return
                # Only allow restart when both players have lost.
            ...
            # Run each player's game (draw the playfield).
            self.game1.run()
            self.game2.run()

            # Draw side panels for both players.
            self.preview1.run(self.next_shapes1)
            self.score1.run()
            self.preview2.run(self.next_shapes2)
            self.score2.run()

            # If both players are finished, update the overlays with the final results.
